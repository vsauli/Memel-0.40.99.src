...
module.exports.extractor = function extractor(Q, reply, glob, socket, ztn) {
	...
	if (task && task.waitList[w] != undefined) {
		...
		for (var sv in task.vars) {
		    if (!task.keepsetvars && !task.vars[sv].cached) task.vars[sv].setvars = false;
...
module.exports.enqueuer = function enqueuer(task, q, ast, precode, code) {
    ...
    var setvars = {};
    for (var sv in task.vars) {
	if (task.vars[sv].setvars == true || task.vars[sv].const == true || task.vars[sv].cached == true) {
	    setvars[sv] = {};
	    for (var svall in task.vars[sv]) {
		if (svall == "value" && task.vars[sv].cached == true) setvars[sv].value = "cached";
		else setvars[sv][svall] = task.vars[sv][svall];
		}
	    }
	}
