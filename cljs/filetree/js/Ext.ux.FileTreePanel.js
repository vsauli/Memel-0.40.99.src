...
Ext.ux.FileTreePanel = Ext.extend(Ext.tree.TreePanel, {
	...
	/**
	 * @cfg {Object} that Parent contaner's scope.
	 */
	that: null
	...
	/**
	 * @cfg {String} createText Create text (for message box title or other displayed texts)
	 */
	,createText:'Create Empty File'
	...
	/**
	 * @cfg {Boolean) enableNewFile true to enable to create new file creation. 
	 * If false context menu item is not shown (defaults to true)
	 */
	,enableNewFile:true
	...
	,initComponent:function() {
		...
		Ext.apply(this, {
			...
			,keys:[{
				...
				// Create Key = Create
				 key:45, stopEvent:true, scope:this
				,fn:function(key, e) {
					var sm, node;
					sm = this.getSelectionModel();
					node = sm.getSelectedNode();
					if(node && this.enableNewFile && this.readOnly !== true) {
...
//			,click:{scope:this, fn:this.onClick}
		...
		this.getSelectionModel().on('selectionchange', function(sc, node) {
		    this.onClick(node);
		}, this);
		...
		this.addEvents(
			...
			/**
			 * @event readfile
			 * Fires after file read has been initiated
			 * @param {Ext.ux.FileTreePanel} this
			 * @param {String} fileName name of the file being opened
			 * @param {String} url url of the file being opened
			 * @param {String} mode open mode
			...
			/**
			 * @event beforecreate
			 * Fires before a new file is created. Return false to cancel the event.
			 * @param {Ext.ux.FileTreePanel} this
			 * @param {Ext.tree.AsyncTreeNode} directory node to add file
			 */
			,'beforecreate'
		...
		if(true === success) {
			...
			if(true === o.success) {
				switch(options.params.cmd) {
					...
					case 'readfile':
						if(true !== this.eventsSuspended) {
							this.fireEvent('readfile', this, this.getPath(options.node));
							this.onReadFile(o.text, o.ext, this.getPath(options.node));
						}
					break;

					case 'savefile':
						if(true !== this.eventsSuspended) {
							this.fireEvent('savefile', this, this.getPath(options.node));
							this.onSaveFile(o.text);
						}
					break;

					case 'create':
						if(true !== this.eventsSuspended) {
							this.fireEvent('create', this, this.getPath(options.node));
							this.readNode(options.node);
							this.getSelectionModel().select(options.node);
						}
//						options.node.parentNode.removeChild(options.node);
					break;
			...
			// process command failure
				...
				switch(options.params.cmd) {
					...
					case 'readfile':
						if(true !== this.eventsSuspended) {
							this.fireEvent('readfilefailure', this, options.node);
						}
					break;

					case 'create':
						if(true !== this.eventsSuspended) {
							this.fireEvent('createfailure', this, options.node);
						}
						options.node.parentNode.reload.defer(1, options.node.parentNode);
					break;
	...
	,createNewFile:function(node) {
		...
		appendNode.expand(false, false, function(n) {
			// create new node
			newNode = n.appendChild(new Ext.tree.AsyncTreeNode({text:'New File', iconCls:'file-txt', leaf: true}));
			// setup one-shot event handler for editing completed
			treeEditor.on({
				complete:{
					 scope:this
...
//			.component;
	...
	,onReadFile(text, ext, path, e) {
	    var extlist = {
		".js":    {syntaxmode: "javascript", save: true, reset: true, run: true,  pane: ["text"], tab: 0},
		".txt":   {syntaxmode: "plain_text", save: true, reset: true, run: false, pane: ["text"], tab: 0},
		".text":  {syntaxmode: "plain_text", save: true, reset: true, run: false, pane: ["text"], tab: 0},
		".html":  {syntaxmode: "html", save: true, reset: true, run: false,  pane: ["text", "html"], tab: 1},
		".htm":   {syntaxmode: "html", save: true, reset: true, run: false,  pane: ["text", "html"], tab: 1},
		".jpg":   {syntaxmode: "text", save: false, reset: false, run: false,  pane: ["image"], tab: 2},
		".jpeg":  {syntaxmode: "text", save: false, reset: false, run: false,  pane: ["image"], tab: 2},
		".gif":   {syntaxmode: "text", save: false, reset: false, run: false,  pane: ["image"], tab: 2},
		".png":   {syntaxmode: "text", save: false, reset: false, run: false,  pane: ["image"], tab: 2},
		".exe":   {syntaxmode: "text", save: false, reset: false, run: false,  pane: ["text"], tab: 0}
		};
	    if (ext == "" || ext == ".") ext = ".txt";
	    if (extlist[ext] == undefined) ext = ".txt";
	    var ex = extlist[ext];
	    this.that.editor.getSession().setValue(' ');
	    var el = Ext.getCmp('image').el;
	    var imgnode = el.dom.childNodes[0].childNodes[0].childNodes[0];
	    imgnode.setAttribute('src', '#');
//console.log(text);
	    if (ex.pane[0] == "text" || ex.pane[1] == "text") {
		this.that.editor.getSession().setValue(text);
		this.that.editor.getSession().getUndoManager().reset();
		this.that.editor.getSession().setMode("ace/mode/" +  ex.syntaxmode);
		this.that.subtab.activate(0);
		this.that.editor.focus();
		this.that.editor.getSession().selection.moveCursorFileStart();
		this.that.modified.setValue('');
		}
	    this.that.tinyMCE.get('elm1').setContent('');
	    
	    if (ex.pane[0] == "html" || ex.pane[1] == "html") {
		this.that.tinyMCE.get('elm1').setContent(text);
	    	this.that.tinyMCE.execCommand('mceReplaceContent',false,text);
		}
		
	    if (ex.pane[0] == "image" || ex.pane[1] == "image") {
		this.that.modified.setValue('');
		el = Ext.getCmp('image').el;
		imgnode = el.dom.childNodes[0].childNodes[0].childNodes[0];
		imgnode.setAttribute('src', path); // .replace(/root/, ""));
		}
		
	    var button = this.that.subtab.find('name', 'save_text')[0];
	    if (ex.save == true) button.enable();
	    else button.disable();

	    var button = this.that.subtab.find('name', 'save_html')[0];
	    if (ex.save == true) button.enable();
	    else button.disable();

	    var button = this.that.subtab.find('name', 'save_reset')[0];
	    if (ex.reset == true) button.enable();
	    else button.disable();

	    var button = this.that.subtab.find('name', 'save_reset_html')[0];
	    if (ex.reset == true) button.enable();
	    else button.disable();

	    var button = this.that.subtab.find('name', 'save_run')[0];
	    if (ex.run == true) button.enable();
	    else button.disable();

	    var button = this.that.subtab.find('name', 'save_run_top')[0];
	    if (ex.run == true) button.enable();
	    else button.disable();

	    if (ex.pane[0] == "html" || ex.pane[1] == "html" || 
	        ex.pane[0] == "image" || ex.pane[1] == "image") {
		this.that.subtab.activate(0);
		var me = this.that;
		setTimeout(function(){me.subtab.activate(ex.tab);}, 500);;
		}
	    else this.that.subtab.activate(ex.tab);
	...
	,readNode:function(node) {
		...
		this.that.filename.setValue(path);
		if(node.isLeaf()) {
