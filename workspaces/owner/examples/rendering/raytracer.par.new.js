...
__remoteDOM("canvas.setAttribute('width',"+WIDTH+");");
__remoteDOM("canvas.setAttribute('height',"+HEIGHT+");");
__remoteDOM("context = canvas.getContext('2d');");
__remoteDOM("imageData = context.getImageData(0,0,"+WIDTH+","+HEIGHT+");");
...
var CHUNK_LEN = Math.floor(HEIGHT / NUM_CHUNKS);
if (HEIGHT % NUM_CHUNKS != 0) NUM_CHUNKS ++;

var CHUNK_START = i * CHUNK_LEN;
var CHUNK_END = (i + 1) * CHUNK_LEN - 1;
if (CHUNK_END >= HEIGHT) CHUNK_END = HEIGHT - 1;
...
for (var y = CHUNK_START; y <= CHUNK_END; y++) {
//    console.log(y);
